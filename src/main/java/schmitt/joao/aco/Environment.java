...
public class Environment {
     ...
     */
    private double[][] balancedPheromone;
     ...
     */
    private double[][] balancedChoiceInfo;
    private double[][] globalChoiceInfo;
    private double[][] localChoiceInfo;
    ...
    public void generateEnvironment() {
        balancedPheromone = new double[getNodesSize()][getNodesSize()];
        globalPheromone = new double[getNodesSize()][getNodesSize()];
        localPheromone = new double[getNodesSize()][getNodesSize()];
        balancedChoiceInfo = new double[getNodesSize()][getNodesSize()];
        localChoiceInfo = new double[getNodesSize()][getNodesSize()];
        globalChoiceInfo = new double[getNodesSize()][getNodesSize()];
        initialTrail = 1.0 / (Parameters.rho * ants[0].calculateNearestNeighborTour());
        for(int i = 0; i < getNodesSize(); i++) {
            for(int j = i; j < getNodesSize(); j++) {
                balancedPheromone[i][j] = initialTrail;
                balancedPheromone[j][i] = initialTrail;
                globalPheromone[i][j] = initialTrail;
                globalPheromone[j][i] = initialTrail;
                localPheromone[i][j] = initialTrail;
                localPheromone[j][i] = initialTrail;

                balancedChoiceInfo[i][j] = initialTrail;
                balancedChoiceInfo[j][i] = initialTrail;
                globalChoiceInfo[i][j] = initialTrail;
                globalChoiceInfo[j][i] = initialTrail;
                localChoiceInfo[i][j] = initialTrail;
                localChoiceInfo[j][i] = initialTrail;
            }
        }
    ...
    public void calculateChoiceInformation() {
        for(int i = 0; i < getNodesSize(); i++) {
            for(int j = 0; j < i; j++) {
                ...
                balancedChoiceInfo[i][j] = Math.pow(balancedPheromone[i][j], 1.0) * Math.pow(heuristic, 2.0);
                balancedChoiceInfo[j][i] = balancedChoiceInfo[i][j];
                localChoiceInfo[i][j] = Math.pow(localPheromone[i][j], 1.0) * Math.pow(heuristic, 5.0);
                localChoiceInfo[j][i] = localChoiceInfo[i][j];
                globalChoiceInfo[i][j] = Math.pow(globalPheromone[i][j], 1.0) * Math.pow(heuristic, .5);
                globalChoiceInfo[j][i] = globalChoiceInfo[i][j];
            }
        }
    }
    ...
    public void generateAntPopulation() {
        ...
        for(int k = 0; k < getAntPopSize(); k++) {
            ants[k] = new Ant(getNodesSize(), this, AntType.Balanced);
            if (k > (int) (0.7 * getAntPopSize())) {
                ants[k] = new Ant(getNodesSize(), this, AntType.Local);
            }
            if (k > (int) (0.9 * getAntPopSize())) {
                ants[k] = new Ant(getNodesSize(), this, AntType.Global);
            }
        }
        /*for(int k = 0; k < getAntPopSize(); k++) {
            ants[k] = new Ant(getNodesSize(), this, AntType.Balanced);
    ...
    public void updatePheromone() {
        ...
        for(int k = 0; k < getAntPopSize(); k++) {
            if(Math.random() < Parameters.migrationRate) {
                Ant a1 = ants[(int) (Math.random() * ants.length)];
                Ant a2 = ants[(int) (Math.random() * ants.length)];
                AntType temp = a1.getAntType();
                a1.setAntType(a2.getAntType());
                a2.setAntType(temp);
            }
    ...
    public void evaporatePheromone() {
        for(int i = 0; i < getNodesSize(); i++) {
            for(int j = i; j < getNodesSize(); j++) {
                balancedPheromone[i][j] = (1 - Parameters.rho) * balancedPheromone[i][j];
                balancedPheromone[j][i] = balancedPheromone[i][j];
            }
        }
    }
    ...
    public void depositPheromone(Ant ant) {
        ...
        for(int i = 0; i < getNodesSize(); i++) {
            ...
            if(AntType.Balanced.equals(ant.getAntType())) {
                balancedPheromone[j][l] = balancedPheromone[j][l] + dTau;
                balancedPheromone[l][j] = balancedPheromone[j][l];
            } else if(AntType.Global.equals(ant.getAntType())) {
                globalPheromone[j][l] = globalPheromone[j][l] + dTau;
                globalPheromone[l][j] = globalPheromone[j][l];
            } else if (AntType.Local.equals(ant.getAntType())) {
                localPheromone[j][l] = localPheromone[j][l] + dTau;
                localPheromone[l][j] = localPheromone[j][l];
            }
        }
    }
     ...
     * @return costInfo
     */
    public double getCostInfo(int from, int to, AntType antType) {
        double cost = 0.0;
        switch (antType) {
            case Local:
                cost = localChoiceInfo[from][to];
                break;
            case Global:
                cost = globalChoiceInfo[from][to];
                break;
            case Balanced:
                cost = balancedChoiceInfo[from][to];
                break;
        }
        return cost;
    }
