...
public class Ant {
     ...
     * @param environment
     */
    public Ant(int tourSize, Environment environment, AntType antType) {
        super();
    ...
    public void goToNNListAsDecisionRule(int phase) {
        ...
        for(int j = 0; j < environment.getNNSize(); j++) {
            if(visited[environment.getNNNode(currentCity, j)]) {
                ...
                selectionProbabilities[j] = environment.getCostInfo(currentCity, environment.getNNNode(currentCity, j), this.getAntType());
    ...
    public void goToBestNeighbor(int phase) {
        ...
        for(int i = 0; i < environment.getNNSize(); i++) {
            ...
            if(!this.visited[helpCity]) {
                help = environment.getCostInfo(currentCity, helpCity, this.getAntType());
                if(help > valueBest) {
                    valueBest = help;
                    nextCity = helpCity;
                }
